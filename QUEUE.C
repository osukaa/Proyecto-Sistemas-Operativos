...
Queue CreateQueue(int MaxElements) {
  ...
  if (MaxElements < MinQueueSize) {
    ...
    printf("CreateQueue Error: Queue size is too small.");
  }

  Q = malloc (sizeof(struct QueueRecord));
  if (Q == NULL) {
    //FatalError("CreateQueue Error: Unable to allocate more memory.");
    printf("CreateQueue Error: Unable to allocate more memory.");
  }
...
ElementType Front(Queue Q) {
  ...
  printf("Front Error: The queue is empty.");
...
ElementType FrontAndDequeue(Queue Q) {
  ...
  if (IsEmpty(Q)) {
    ...
    printf("FrontAndDequeue Error: The queue is empty.");
  } else {
    Q->Size--;
    X = Q->Array[Q->Front];
    Q->Front = Succ(Q->Front, Q);
